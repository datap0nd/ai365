...
# Design System: MetoAI

## Overview

**Creative North Star: "The Guardrailed Desk Tool"**

MetoAI is a compact native Outlook sidebar. It should feel native to classic Outlook, not like an AI showcase: familiar system typography, quiet white and cool-gray surfaces, square fields, restrained density, and blue reserved for direct actions.
...
- **Title** (bold, nominally 12pt): MetoAI in the top strip.
...
- **Structure:** Muted full-width band with a bold "MetoAI" title, an ellipsized optional `Selected: subject` or `Working set: N of 10 emails`, and exactly `Model: model_name`.
...
- **Accessibility:** Accessible name is "MetoAI conversation"; the description identifies it as a plain-text mailbox conversation and context-loading ledger.
...
- **Instruction:** A persistent safety line explains how to request a draft and states that MetoAI cannot send. A separate hint says Ctrl+Enter submits the chat prompt.
